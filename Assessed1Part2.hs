...
makeTree trees = makeTree' trees 
                
makeTree' :: [Tree c] -> Tree c
makeTree' [x] = x
makeTree' trees = makeTree'( insertBy (comparing freq) (merge (head(mySort trees)) (head(tail(mySort trees)))) ( drop 2 (mySort trees)))

copac = makeTree([Leaf 'a' 5, Leaf 'b' 7, Leaf 'c' 10, Leaf 'd' 15, Leaf 'e' 20, Leaf 'f' 45])
frecvente = [('a',5), ('b',7), ('c',10), ('d',15), ('e',20), ('f',45)]

mySort :: [Tree c] -> [Tree c]
mySort trees = sortOn freq trees

-- You may wish to use a helper function such as this:
merge :: Tree c -> Tree c -> Tree c
merge left right = Branch left right (freq left + freq right) 
...
makeTable' (Branch l r i) xs = makeTable' l (xs ++ [Z])
                            ++ makeTable' r (xs ++ [I])
makeTable' (Leaf c i) xs = [(c, xs)]


tabel = makeTable copac

-- Question:
-- Takes a string of symbols to a bit string, based on a given coding table
encodeUsingTable :: Eq c => CodingTable c -> [c] -> [Bit]
encodeUsingTable codingTable str = concat [encodeSymbolUsingTable c codingTable | c <- str]

encodeSymbolUsingTable :: Eq c => c -> CodingTable c -> [Bit]
encodeSymbolUsingTable c table = case found of Just a -> snd a
                                               Nothing -> undefined
                                 where found = find (\x -> fst x == c) table

-- Question:
-- Encodes directly from the tree (more efficient).
encodeUsing :: Eq c => Tree c -> [c] -> [Bit]
encodeUsing tree str = concat [encodeUsing' x tree [] | x <- str]

encodeUsing' :: Eq c => c -> Tree c -> [Bit] -> [Bit]
encodeUsing' x (Branch l r i) xs = encodeUsing' x l (xs ++ [Z])
                            ++ encodeUsing' x r (xs ++ [I])
encodeUsing' x (Leaf c i) xs = if (c == x) then xs else []
...
compress string = show(length (show(fst(encode string)))) ++ show(fst(encode string)) ++ show(snd(encode string))

-- Question:
-- Smarter compression: if the encoded string is larger than the input string,
-- instead output the input string with a '*' in front.
compress' :: String -> String
compress' string = if(memSize string < memSize(show(fst(encode string))) + length(show(snd(encode string))) + memSize(show(length (show(fst(encode string)))))) then '*' : string else compress string 
